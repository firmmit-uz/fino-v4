<script lang="ts">
  ...
  const STEPS = [
    { ko: '탱크에 깨끗한 물 60–70% 채우기',  uz: 'Idishga toza suv 60–70% quying',  ru: 'Заполнить бак чистой водой 60–70%' },
    ...
    { ko: 'C탱크 산을 별도 희석 후 천천히 첨가 ⚠', uz: 'C kislotasini alohida suyultirib quying ⚠', ru: 'Кислоту бака C разбавить отдельно, добавить медленно ⚠' },
...
<div class="bg-white text-black min-h-screen p-8 max-w-3xl mx-auto font-sans">
  ...
  <div class="border-b-4 border-green-600 pb-4 mb-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <div class="text-sm text-gray-500">본격재배 · Main Crop · Asosiy ekin</div>
      </div>
      ...
      {#if tanks && Object.keys(tanks[tk]).length > 0}
        <div class="border-2 {tk==='A'?'border-blue-500':tk==='B'?'border-green-500':'border-amber-500'} rounded-xl p-3">
          <div class="text-center font-black text-2xl mb-2
            {tk==='A'?'text-blue-700':tk==='B'?'text-green-700':'text-amber-700'}">{tk}탱크</div>
          ...
          {/each}
          {#if tk === 'C'}
            <div class="mt-2 text-xs text-amber-700 font-bold">⚠ 원액 1:100 희석 필수</div>
          {/if}
        </div>
      {/if}
    {/each}
  ...
  <!-- 7-step procedure — trilingual -->
  <div class="mb-6">
    <div class="text-lg font-bold mb-3 text-gray-800">조제 절차 · Procedure · Тайёрлаш</div>
    <div class="space-y-3">
  ...
  <div class="border border-gray-200 rounded-xl overflow-hidden">
    <div class="bg-gray-50 px-4 py-2 text-xs font-bold text-gray-600">목표 이온 농도 (mmol/L)</div>
  ...
  <div class="mt-4 text-xs text-gray-400 text-right">
    FINO v4 · Tashkent default · {new Date().toLocaleDateString()}
  </div>
  {/if}
</div>
