<script lang="ts">
  ...
  const modifiedTarget = $derived((): TargetIons | undefined => {
    ...
    const { variety: vMod, focus: fMod, season } = $settings;
    const ef = WINTER_FACTOR[season];
    const tg = stage.target;
    return {
      ...tg,
      NO3:   tg.NO3   * vMod.no3Mod * fMod.no3Mod * ef,
      NH4:   tg.NH4   * ef,
      K:     tg.K     * vMod.kMod   * fMod.kMod,
      Ca:    tg.Ca    * vMod.caMod  * fMod.caMod,
      H2PO4: tg.H2PO4 * fMod.h2po4Mod,
      Mg:    tg.Mg,
      SO4:   tg.SO4,
    };
  });
...
<div class="min-h-screen bg-bg pb-8">
  ...
  <div class="px-4 py-4">
    <div class="flex items-start justify-between mb-1">
      <div>
        ...
        {#if stage.dates}
          <div class="text-xs text-nursery font-semibold">{stage.dates} · {stage.days} {$t('unit.days_short')}</div>
        {/if}
        <div class="text-sm text-ink3">EC {stage.ecMin}–{stage.ecMax} {$t('unit.ms_cm')}</div>
      </div>
      <a href="/print/nursery/{stageId}" target="_blank"
         class="px-3 py-1.5 rounded-xl bg-nursery-surface text-nursery text-sm font-bold">
        🖨 {$t('nav.print')}
      </a>
    </div>
  </div>
  ...
  <div class="px-4 flex flex-col gap-4">
    ...
    <button onclick={() => showRawEdit = !showRawEdit}
            class="text-sm font-semibold text-nursery text-left">
