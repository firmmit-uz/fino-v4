<script lang="ts">
  import { t, setLang, currentLang, type Lang } from '$lib/i18n/index.js';
  ...
  let lang = $state(currentLang() as Lang);
...
<div class="min-h-screen bg-slate-900 text-white flex flex-col">
  ...
  <div class="px-4 mb-6">
    <div class="flex gap-2 overflow-x-auto pb-1">
      {#each LANGS as lg}
        <button
          onclick={() => selectLang(lg.code)}
          class="flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-semibold border transition-colors
            {lang === lg.code ? 'bg-green-600 border-green-500 text-white' : 'border-slate-600 text-slate-300 hover:border-slate-400'}"
        >{lg.label}</button>
      {/each}
    </div>
  </div>
  ...
  <div class="px-4 flex flex-col gap-4 flex-1 pb-8 max-w-lg mx-auto w-full">
    <button onclick={() => goto('/nursery')} class="w-full rounded-2xl overflow-hidden text-left">
      ...
      <div class="bg-green-950 px-6 py-3 flex items-center justify-between">
        <span class="text-green-300 text-sm font-semibold">EC 0.4–1.7 mS/cm</span>
    ...
    <button onclick={() => goto('/main')} class="w-full rounded-2xl overflow-hidden text-left">
      ...
      <div class="bg-blue-950 px-6 py-3 flex items-center justify-between">
        <span class="text-blue-300 text-sm font-semibold">EC 0.8–1.6 mS/cm</span>
        <span class="text-blue-400">→</span>
      </div>
    </button>

    <div class="rounded-xl bg-slate-800 border border-slate-700 p-4 text-xs text-slate-400">
      <div class="font-bold text-slate-300 mb-1">원수 · Tashkent default (AKIS 2026)</div>
      <div>pH 7.6 · EC 0.83 · HCO₃ 3.3 · Ca 2.0 · Mg 0.5 mmol/L</div>
    </div>
  </div>
</div>
